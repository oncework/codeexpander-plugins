...
import { Button, Card, CardContent, CardHeader, CardTitle, Checkbox, Input, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@codeexpander/dev-tools-ui';
...
const FakeDataGenerator = () => {
  ...
  return (
    <div className="p-6 space-y-6">
      <Card>
        <CardHeader>
          <CardTitle className="text-lg">{t('fakeDataGenerator.dataFieldsTitle')}</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            {Object.entries(options).map(([key, value]) => (
              <div key={key} className="flex items-center space-x-2">
                ...
                <label htmlFor={key} className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70 capitalize">
                  {t(`fakeDataGenerator.${fieldLabelKeys[key] || key}`)}
                </label>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-2">{t('fakeDataGenerator.recordsCountLabel')}</label>
          <Input
            type="number"
            min="1"
            max="100"
        ...
        <div>
          <label className="block text-sm font-medium mb-2">{t('fakeDataGenerator.outputFormatLabel')}</label>
          <Select value={format} onValueChange={setFormat}>
            <SelectTrigger>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
              <SelectItem value="json">{t('fakeDataGenerator.json')}</SelectItem>
              <SelectItem value="csv">{t('fakeDataGenerator.csv')}</SelectItem>
              <SelectItem value="sql">{t('fakeDataGenerator.sql')}</SelectItem>
            </SelectContent>
          </Select>
        </div>
      </div>
      ...
      >
        {t('fakeDataGenerator.generateBtn')}
      </Button>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="text-lg flex justify-between items-center">
              {t('fakeDataGenerator.generatedTitle')}{format.toUpperCase()})
              <Button
                variant="outline"
                size="sm"
                onClick={() => copyWithToast(generatedData, t('common.copied'))}
              >
                {t('common.copy')}
              </Button>
            </CardTitle>
      ...
      <Card>
        <CardContent className="p-4">
          <div className="text-sm text-slate-600">
            <p><strong>{t('fakeDataGenerator.useCases')}</strong> {t('fakeDataGenerator.useCasesText')}</p>
            <p className="mt-2"><strong>{t('fakeDataGenerator.note')}</strong> {t('fakeDataGenerator.noteText')}</p>
          </div>
        </CardContent>
      </Card>
    </div>
