  ...
  const convertImage = async () => {
    ...
    try {
      ...
      img.onload = () => {
        ...
        showToast(`Conversion successful: Image converted to ${formatName}`);
      };

      img.onerror = () => {
        showErrorToast("Conversion failed", "Failed to load the image.");
      };

      img.src = URL.createObjectURL(selectedFile);
    } catch (error) {
      showErrorToast("Conversion failed", "An error occurred during conversion.");
    } finally {
