...
const TemperatureConverter = () => {
  ...
  const convertTemperature = () => {
    ...
    if (isNaN(inputTemp)) {
      showErrorToast(t("common.error"), t("temperatureConverter.errorInvalid"));
      return;
    }
  ...
  const copyResult = (value: number, scale: string) => {
    ...
    copyToClipboard(`${value.toFixed(6)} ${scaleInfo?.symbol ?? ""}`, t("common.copied"));
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6">
      <Card>
        <CardHeader>
          <CardTitle>{t("temperatureConverter.title")}</CardTitle>
          <CardDescription>
            {t("temperatureConverter.description")}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <Button onClick={convertTemperature} className="w-full">
            {t("temperatureConverter.convertBtn")}
          </Button>
