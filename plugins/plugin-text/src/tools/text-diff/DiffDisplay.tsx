    ...
    <Card className="mt-6">
      <CardHeader className="pb-4">
        <CardTitle className="text-lg flex items-center justify-between">
          {t("textDiff.differences")}
          <div className="text-sm font-normal text-slate-600 dark:text-slate-400">
            <span className="text-green-600">
              +{diff.filter((d) => d.type === "added").length} {t("textDiff.added")}
            </span>
            {" | "}
            <span className="text-red-600">
              -{diff.filter((d) => d.type === "removed").length} {t("textDiff.removed")}
            </span>
            {" | "}
            <span className="text-slate-600">
              {diff.filter((d) => d.type === "unchanged").length} {t("textDiff.unchanged")}
            </span>
          </div>
        </CardTitle>
      </CardHeader>
        ...
        <div className="space-y-2 font-mono text-sm max-h-96 overflow-y-auto border border-slate-200 dark:border-slate-700 rounded-lg p-4 bg-slate-50 dark:bg-slate-800">
          {diff.map((item, index) => (
            ...
            >
              ...
              {item.inlineChanges ? (
                <span>
                  <InlineChanges changes={item.inlineChanges} />
                </span>
              ) : (
                <span>{item.value || t("textDiff.emptyLine")}</span>
              )}
            </div>
          ))}
        </div>
