...
const DomainStatsCards = ({ domainData }: DomainStatsCardsProps) => {
  ...
  const getTrendText = (trend: number | null) => {
    if (trend === null) return t("websiteRankTracker.noTrendData");
    if (trend === 0) return t("websiteRankTracker.noChange");
    if (trend > 0) return `+${trend}` + t("websiteRankTracker.rankDecreased");
    return `${trend}` + t("websiteRankTracker.rankImproved");
  };
