...
- 针对**「以插件窗口打开」**独立窗，还支持：
  ...
  - **permissions**（string 数组）— 插件运行时需要的 Tauri 权限标识。宿主在展示插件窗口时通过 `add_capability` 动态授予。需声明完整标识，如 `core:window:allow-set-size`、`core:window:allow-set-position`。若省略或为空，不授予任何额外权限。示例：
