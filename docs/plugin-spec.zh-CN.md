...
用于 **title**、**description**、**category** 的多语言文案。宿主按当前语言（如 `zh_CN`、`en`）解析展示。

- **结构**：`i18n` 为对象，key 为语言码（`en`、`zh_CN` 等），value 为「字段名或 key → 文案」对象（如 `title`、`description`、`category`）。
- **解析顺序**（每个字段）：(1) 若存在 `i18n[locale].field` 或 `i18n.en.field` 则用；(2) 若主字段形如 `{{key}}`，则在 `i18n[locale]` / `i18n.en` 中查该 key；(3) 否则用主字段值。
