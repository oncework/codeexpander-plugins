...
function isThirdPartyPlugin(name) {
  ...
  return entries.some((e) => e.isDirectory() && existsSync(join(dir, e.name, '.git')))
}

function discoverPlugins() {
  return readdirSync(PLUGINS_DIR, { withFileTypes: true })
    .filter((d) => d.isDirectory() && d.name.startsWith('plugin-') && isThirdPartyPlugin(d.name))
    .map((d) => d.name)
    .sort()
}
...
function resolvePluginNames(input, allPlugins) {
  ...
  for (const r of raw) {
    ...
    if (allPlugins.includes(name)) {
      ...
      console.error(`Available: ${allPlugins.map((p) => p.replace('plugin-', '')).join(', ')}`)
      process.exit(1)
    }
  }
  return resolved
...
function buildPlugin(name) {
  ...
  try {
    ...
    const msg = err.status !== null ? `exit code ${err.status}` : err.signal || 'unknown error'
...
async function main() {
  ...
  if (failed.length > 0) {
    ...
    for (const p of failed) {
      console.log(`  ❌ ${p.replace('plugin-', '')} — ${state[p].error || 'unknown'}`)
    }
  }
