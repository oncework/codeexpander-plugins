...
import { Card, CardContent, CardDescription, CardHeader, CardTitle, Button, Input, Label, Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from "@codeexpander/dev-tools-ui";
...
const RandomPhoneGenerator = () => {
  ...
  const generatePhoneNumber = (country: string) => {
    ...
    if (country === "US" || country === "Canada") {
      phone = phone.replace(/\+1 \(([01])\d{2}\)/, (match, firstDigit) => {
        const newFirstDigit = Math.floor(Math.random() * 8) + 2;
  ...
  const copyAllToClipboard = () => {
    ...
    copyWithToast(allNumbers, t("common.copied"));
  };
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-6">
      <Card>
        <CardHeader>
          <CardTitle>{t("randomPhoneGenerator.title")}</CardTitle>
          <CardDescription>
            {t("randomPhoneGenerator.description")}
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div className="space-y-2">
              <Label htmlFor="country">{t("randomPhoneGenerator.countryLabel")}</Label>
              <Select value={selectedCountry} onValueChange={setSelectedCountry}>
                <SelectTrigger>
                  <SelectValue placeholder={t("randomPhoneGenerator.countryPlaceholder")} />
                </SelectTrigger>
                <SelectContent>
            ...
            <div className="space-y-2">
              <Label htmlFor="count">{t("randomPhoneGenerator.countLabel")}</Label>
              <Input
                id="count"
                type="number"
                min="1"
            ...
            <div className="space-y-2">
              ...
              <Button onClick={handleGenerate} className="w-full">
                ...
                {t("randomPhoneGenerator.generateBtn")}
              </Button>
            </div>
          </div>

          <div className="space-y-2">
            <Label>{t("randomPhoneGenerator.formatLabel")}{phoneFormats[selectedCountry as keyof typeof phoneFormats]?.format}</Label>
          </div>

          {phoneNumber && (
            <div className="space-y-2">
              <Label>{t("randomPhoneGenerator.singleLabel")}</Label>
          ...
          {phoneNumbers.length > 0 && (
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <Label>{t("randomPhoneGenerator.multipleLabel")}{phoneNumbers.length}{t("randomPhoneGenerator.multipleLabelSuffix")}</Label>
                <Button
                  variant="outline"
                  size="sm"
                ...
                >
                  ...
                  {t("randomPhoneGenerator.copyAll")}
                </Button>
              </div>
